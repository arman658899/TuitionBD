...
public class ProfileInteractor {
    ...
    public void createUserHashMap(final HashMap<String,Object> hashMap, final OnUploadListener listener){
        /*String documentId = getUserDocumentId();
        if (documentId == null) {
            listener.onFailed();
            return;
        ...
        if (hashMap.containsKey("documentId")){
            collUsers.document(String.valueOf(hashMap.get("documentId")))
                    .set(hashMap)
                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
